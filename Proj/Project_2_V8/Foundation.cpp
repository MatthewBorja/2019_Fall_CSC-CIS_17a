...
bool Foundation::addToC(unsigned char crd){
    Card candidate(crd);            //Create a card object for suggested move.
    if(legalCheckC(candidate)){     //Send it in to see if legal move.
        c[numOfCCrds]=crd;          //If legal, assign to foundation.
        if(numOfCCrds<12){
...
bool Foundation::addToD(unsigned char crd){
    Card candidate(crd);
...
bool Foundation::addToH(unsigned char crd){
    Card candidate(crd);
...
bool Foundation::addToS(unsigned char crd){
    Card candidate(crd);
