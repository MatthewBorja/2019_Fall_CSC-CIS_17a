...
Deck::Deck(unsigned char num){
    ...
    indx=new unsigned char[nCards];  //Set indx = Dynamically allocate 52 new char pointers.
...
unsigned char *Deck::deal(unsigned char nDeal){
    shuffle();
    nDealt=0;
    unsigned char *pile=new unsigned char[nDeal];
